...
const printBordereau = (order) => {
  ...
  const html = `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Bordereau ${order.id}</title>
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    th { background: #f5f5f5; padding: 6px 8px; text-align: left; font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px; }
  ...
  <div class="page">
    ...
    <div class="section">
      ...
      <table>
        <thead><tr><th>Désignation</th><th>Qté</th><th>Prix</th></tr></thead>
        <tbody>${produitsList}</tbody>
        <tfoot><tr class="total-row"><td colspan="2">Total commande</td><td>${total} €</td></tr></tfoot>
      </table>
    </div>
    ...
    <div class="footer">
      ACA Wholesale • Moselle, France • contact@aca-wholesale.com<br>
      Document généré automatiquement — À coller sur le colis
    </div>
  </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-8">
        {stats.map(stat => (
          <button key={stat.label} onClick={() => setActiveTab(stat.tab)} className="rounded-xl p-5 text-left hover:border-[#C4962A]/40 transition-all" style={{ background: 'rgba(15,10,0,0.85)', border: '1px solid rgba(255,255,255,0.07)' }}>
      ...
      {aExpedier > 0 && (
        <div className="rounded-xl p-5 flex items-center justify-between" style={{ background: 'rgba(196,150,42,0.1)', border: '1px solid rgba(196,150,42,0.3)' }}>
          ...
          <button onClick={() => setActiveTab('commandes')} className="text-black text-xs px-4 py-2 font-black uppercase tracking-wide rounded" style={{ background: 'linear-gradient(135deg, #C4962A, #E8B84B)' }}>
            Voir →
          </button>
        </div>
      )}
...
function CommandesTab() {
  ...
  const filtered = filtre === 'Tous' ? mockOrders : mockOrders.filter(o => o.status === filtre)
  ...
  if (selected) {
    ...
    return (
      <div>
        <button onClick={() => setSelected(null)} className="flex items-center gap-2 text-gray-400 hover:text-white text-xs font-bold uppercase tracking-wide mb-6 transition-colors">
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          ...
          <div className="rounded-xl p-5" style={{ background: 'rgba(15,10,0,0.85)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { l: 'Poids', v: `${order.poids} kg` },
                { l: 'Dimensions', v: `${order.dimensions} cm` },
                { l: 'Valeur', v: `${order.valeur} €` },
                { l: 'Livraison', v: order.livraison },
              ].map(i => (
            ...
            {order.produits.map((p, i) => (
              <div key={i} className="flex justify-between items-center py-2 border-b border-white/5">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-gray-400 text-xs">x{p.qte}</span>
                  <span className="text-white font-bold text-sm">{p.prix * p.qte} €</span>
                </div>
              </div>
            ))}
        ...
        <button
          ...
          className="w-full py-4 font-black text-base uppercase tracking-widest text-black rounded-xl transition-opacity hover:opacity-90 flex items-center justify-center gap-3"
    ...
    <div>
      ...
      <div className="space-y-3">
        {filtered.map(order => {
          const total = order.produits.reduce((s, p) => s + p.prix * p.qte, 0)
          return (
            <div key={order.id} onClick={() => setSelected(order.id)} className="rounded-xl p-4 cursor-pointer hover:border-[#C4962A]/40 transition-all" style={{ background: 'rgba(15,10,0,0.85)', border: '1px solid rgba(255,255,255,0.07)' }}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div>
                    ...
                    <p className="text-gray-400 text-xs">{order.client.nom} • {order.client.ville} • {order.date}</p>
                ...
                <div className="text-right flex-shrink-0 ml-4">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); printBordereau(order) }} className="text-[10px] font-bold px-3 py-1 rounded mt-1 uppercase tracking-wide text-black transition-opacity hover:opacity-80" style={{ background: 'linear-gradient(135deg, #C4962A, #E8B84B)' }}>
                    🖨️ Imprimer
                  </button>
                </div>
              </div>
