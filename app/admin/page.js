...
function buildBordereauHTML(order) {
  const lignes = order.articles.map(a =>
    '<tr>' +
    '<td style="padding:6px 10px;border-bottom:1px solid #eee">' + a.nom + '</td>' +
    '<td style="padding:6px 10px;border-bottom:1px solid #eee;text-align:center">' + a.taille + '</td>' +
    '<td style="padding:6px 10px;border-bottom:1px solid #eee;text-align:center">' + a.qty + '</td>' +
    '<td style="padding:6px 10px;border-bottom:1px solid #eee;text-align:right">' + a.prix + '€</td>' +
    '</tr>'
  ).join('')
  return (
...
function printMultiple(orders) {
  var contenu = orders.map(function(o) { return '<div class="bordereau">' + buildBordereauHTML(o) + '</div>' }).join('')
  var html = '<!DOCTYPE html><html><head><title>Bordereaux</title><style>body{margin:0;padding:20px;background:#fff}.bordereau{margin-bottom:40px;page-break-after:always}@media print{body{padding:0}.bordereau{margin:0;page-break-after:always}}</style></head><body>' + contenu + '<script>window.onload=function(){window.print()}<\/script></body></html>'
  ...
  return (
    <div className="flex flex-col items-center justify-center h-96">
      <div className="text-center p-10 rounded-2xl border border-yellow-600/30" style={{background:'rgba(196,150,42,0.07)'}}>
        ...
        <button className="px-8 py-3 rounded-full font-bold text-black text-base" style={{background:'linear-gradient(135deg,#C4962A,#E8B84B)'}}>Mettre à niveau</button>
      </div>
    </div>
  )
}
...
function DashboardHome() {
  const aExpedier = mockOrders.filter(function(o) { return o.statut === 'à expédier' }).length
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        {[
          ...
          { label: 'Chiffre du mois', value: mockOrders.reduce(function(s,o){return s+o.total},0) + '€', icon: '💰', color: '#55c455' },
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <div className="flex gap-1 flex-wrap justify-end">
                {statutOptions.filter(function(s){return s !== detail.statut}).map(function(s) {
                  return (
                    <button key={s} onClick={function(){changeStatut(detail.id, s)}} className="px-3 py-1 rounded-full text-xs border border-white/20 hover:border-yellow-500/50 transition-all" style={{background:'rgba(255,255,255,0.07)', color:'#ccc'}}>
                      → {s}
                    </button>
                  )
                })}
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-4 flex-wrap items-center">
        ...
        {selected.length > 0 && (
          <button onClick={function(){printMultiple(selectedOrders)}} className="ml-auto px-5 py-1.5 rounded-lg font-bold text-black text-sm" style={{background:'linear-gradient(135deg,#C4962A,#E8B84B)'}}>
...
function ClientsTab() {
  ...
  const filteredClients = mockClients.filter(function(c) { return segmentFiltre === 'tous' || c.segment === segmentFiltre })
  return (
    <div>
      <h2 className="text-2xl font-bold text-yellow-400 mb-6">Clients</h2>
      <div className="flex gap-2 mb-4 flex-wrap">
        {['tous','VIP','Actif','Nouveau'].map(function(s) {
          return (
            <button key={s} onClick={function(){setSegmentFiltre(s)}} className={'px-3 py-1 rounded-full text-xs font-medium border transition-all ' + (segmentFiltre === s ? 'text-black border-yellow-500' : 'text-gray-300 border-white/20')} style={segmentFiltre === s ? {background:'linear-gradient(135deg,#C4962A,#E8B84B)'} : {}}>{s}</button>
          )
        })}
      </div>
      <div className="rounded-xl overflow-hidden border border-white/10">
            ...
            {filteredClients.map(function(c) {
              return (
                <tr key={c.id} className="border-t border-white/5 hover:bg-white/5">
                  ...
                  <td className="p-3">
                    <span className={'px-2 py-0.5 rounded-full text-xs font-medium ' + (c.segment === 'VIP' ? 'bg-yellow-500/20 text-yellow-400' : c.segment === 'Actif' ? 'bg-blue-500/20 text-blue-400' : 'bg-gray-500/20 text-gray-400')}>{c.segment}</span>
                  </td>
                </tr>
              )
            })}
    ...
    <div className="min-h-screen flex" style={{background:'#0a0500'}}>
      <aside className={(sidebarOpen ? 'w-64' : 'w-16') + ' transition-all duration-300 flex flex-col border-r border-white/10'} style={{background:'rgba(255,255,255,0.03)'}}>
        <div className="p-4 flex items-center justify-between border-b border-white/10">
          ...
          <button onClick={function(){setSidebarOpen(!sidebarOpen)}} className="text-gray-400 hover:text-white p-1">{sidebarOpen ? '◀' : '▶'}</button>
        </div>
        <nav className="flex-1 p-3 space-y-1">
          {navItems.map(function(item) {
            return (
              <button key={item.id} onClick={function(){setActiveTab(item.id)}} className={'w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-left ' + (activeTab === item.id ? 'text-black font-medium' : 'text-gray-300 hover:text-white hover:bg-white/5')} style={activeTab === item.id ? {background:'linear-gradient(135deg,#C4962A,#E8B84B)'} : {}}>
