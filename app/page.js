    ...
    <main className="bg-black overflow-x-hidden">
      ...
      <section className="relative bg-black text-white overflow-hidden min-h-[80vh] md:min-h-screen flex items-center">
        ...
        <div className="relative max-w-7xl mx-auto px-5 py-20 md:py-32 w-full">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 border border-white/20 text-gray-400 text-[10px] font-black px-3 py-1.5 mb-6 uppercase tracking-widest">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Link href="/produits" className="text-black px-8 py-4 font-black text-xs uppercase tracking-widest text-center hover:opacity-90 transition-all" style={{ background: 'linear-gradient(135deg, #C4962A, #E8B84B)' }}>
                VOIR NOS LOTS →
              </Link>
              <Link href="/a-propos" className="border border-white/20 text-white px-8 py-4 font-bold text-xs uppercase tracking-widest hover:border-white/50 transition-colors text-center">
                QUI SOMMES-NOUS ?
              </Link>
            </div>
            <div className="flex gap-6 md:gap-10 mt-12 pt-12 border-t border-white/10">
              {[
                ...
                <div key={s.label} className="text-center">
                  ...
                  <div className="text-gray-500 text-[9px] md:text-xs mt-1 uppercase tracking-widest">{s.label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-0 md:divide-x divide-white/10">
            {[
              ...
              <div key={item.title} className="text-center px-4 py-2">
                ...
                <div className="font-black text-xs text-white uppercase tracking-widest">{item.title}</div>
      ...
      <section className="py-12 md:py-20">
        <div className="max-w-7xl mx-auto px-5">
          <div className="flex items-end justify-between mb-6 md:mb-10">
            ...
            <Link href="/produits" className="text-xs font-black uppercase tracking-widest text-gray-500 hover:text-white transition-colors">VOIR TOUT →</Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
            {newDrops.map((product) => (<ProductCard key={product.id} product={product} />))}
          </div>
          <div className="text-center mt-6 md:hidden">
            <Link href="/produits" className="inline-block border border-white/20 text-white px-6 py-3 font-black text-xs uppercase tracking-widest">Tout afficher →</Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 md:py-20 border-t border-white/10">
        <div className="max-w-7xl mx-auto px-5">
          <div className="flex items-end justify-between mb-6 md:mb-10">
            ...
            <Link href="/produits" className="text-xs font-black uppercase tracking-widest text-gray-500 hover:text-white transition-colors">SHOP ALL →</Link>
          </div>
          <div className="grid grid-cols-3 lg:grid-cols-6 gap-2 md:gap-3">
            {categories.map((cat) => (
              <Link key={cat.name} href="/produits" className="group text-center p-4 md:p-6 bg-[#111] border border-white/07 hover:border-[#C4962A]/50 transition-all duration-300" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
                <div className="text-2xl md:text-3xl mb-2 md:mb-3">{cat.emoji}</div>
                <h3 className="font-black text-[10px] text-white uppercase tracking-widest mb-0.5">{cat.name}</h3>
                <p className="text-[10px] text-gray-600">{cat.count} lots</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="py-12 md:py-20 border-t border-white/10">
        <div className="max-w-7xl mx-auto px-5">
          <div className="flex items-end justify-between mb-6 md:mb-10">
            <div>
              <span className="inline-block text-[10px] font-black px-2 py-0.5 uppercase tracking-widest mb-3" style={{ background: 'linear-gradient(135deg, #C4962A, #E8B84B)', color: '#000' }}>⭐ POPULAIRES</span>
      ...
      <section className="py-10 md:py-16 border-y border-white/10 overflow-hidden">
        <div className="max-w-7xl mx-auto px-5 mb-6">
          <p className="text-gray-600 text-[10px] uppercase tracking-widest font-black text-center">Marques présentes dans nos lots</p>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/10">
            {[
              ...
              { step: '03', title: 'Revendez à la pièce', desc: 'Recevez votre lot et revendez chaque pièce sur Vinted. Multipliez votre investissement !', emoji: '🚀' },
            ].map((item) => (
              <div key={item.step} className="bg-black p-6 md:p-10 text-center">
                <div className="text-3xl md:text-4xl mb-4">{item.emoji}</div>
                <div className="text-[10px] font-black uppercase tracking-widest mb-2" style={{ color: '#C4962A' }}>ÉTAPE {item.step}</div>
                <h3 className="text-sm md:text-base font-black text-white mb-3 uppercase tracking-wide">{item.title}</h3>
      ...
      <section className="py-12 md:py-20 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-5">
          <div className="grid md:grid-cols-2 gap-px bg-white/10 overflow-hidden">
            <div className="bg-black p-6 md:p-12">
              <div className="text-[10px] font-black uppercase tracking-widest mb-3" style={{ color: '#C4962A' }}>Pourquoi ACA Wholesale ?</div>
              ...
              <p className="text-gray-400 leading-relaxed mb-4 text-sm">Après plusieurs années dans la revente, nous avons créé ACA Wholesale pour répondre à un vrai besoin : proposer des lots de qualité, sélectionnés avec soin, avec un bon potentiel de revente.</p>
              ...
              <div className="space-y-2 mb-8">
                {['Sélection rigoureuse des produits', 'Expédition rapide depuis la France', 'Lots pensés pour la revente sur Vinted', 'Relation de confiance durable'].map((item) => (
                  <div key={item} className="flex items-center gap-3">
                    <span className="w-5 h-5 flex items-center justify-center text-[10px] font-black flex-shrink-0" style={{ background: 'linear-gradient(135deg, #C4962A, #E8B84B)', color: '#000' }}>✓</span>
                    <span className="text-xs font-bold text-gray-300 uppercase tracking-wide">{item}</span>
                  </div>
                ))}
              </div>
              <Link href="/a-propos" className="inline-block border border-white/20 text-white px-6 py-2.5 font-black text-xs uppercase tracking-widest hover:border-white/50 transition-colors">En savoir plus →</Link>
            </div>
            <div className="bg-[#0a0a0a] p-6 md:p-12 flex items-center justify-center min-h-[300px]">
              <div className="text-center">
                <div className="text-6xl mb-6">📦</div>
                <p className="text-white font-black text-base uppercase tracking-wide">Notre entrepôt en Moselle</p>
                ...
                <div className="mt-8 grid grid-cols-2 gap-3">
                  {[{ emoji: '✋', text: 'Tri manuel' }, { emoji: '🔍', text: 'Contrôle qualité' }, { emoji: '📏', text: 'Tailles variées' }, { emoji: '🚚', text: 'Envoi rapide' }].map(b => (
                    <div key={b.text} className="bg-black border border-white/10 p-3 text-center">
                      <div className="text-lg mb-1">{b.emoji}</div>
                      <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{b.text}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="py-12 md:py-20 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-5">
          <div className="flex items-end justify-between mb-8 md:mb-12">
            ...
            <p className="text-gray-600 text-[10px] uppercase tracking-widest font-black">Ce que disent nos revendeurs</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-4">
            {testimonials.map((t, i) => (
              <div key={i} className="bg-[#111] border border-white/07 p-6 md:p-8" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-white/10">
                  <div className="w-9 h-9 flex items-center justify-center font-black text-sm text-black" style={{ background: 'linear-gradient(135deg, #C4962A, #E8B84B)' }}>{t.name.charAt(0)}</div>
                  <div>
                    <div className="font-black text-xs text-white uppercase tracking-wide">{t.name}</div>
      ...
      <section className="py-12 md:py-20 border-b border-white/10">
        <div className="max-w-3xl mx-auto px-5">
          <div className="flex items-end justify-between mb-8 md:mb-12">
            ...
            <Link href="/faq" className="text-xs font-black uppercase tracking-widest text-gray-500 hover:text-white transition-colors">TOUT VOIR →</Link>
          </div>
            ...
            {faqItems.map((item, i) => (
              <div key={i} className="faq-item">
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex items-center justify-between p-4 md:p-5 text-left">
                  <span className="font-black text-xs text-white pr-4 uppercase tracking-wide">{item.q}</span>
                  <span className="text-xl flex-shrink-0 font-black" style={{ color: '#C4962A' }}>{openFaq === i ? '−' : '+'}</span>
                </button>
      ...
      <section className="py-16 md:py-24">
        <div className="max-w-4xl mx-auto px-5 text-center">
          <div className="border border-white/10 p-10 md:p-16">
            <p className="text-[10px] uppercase tracking-widest text-gray-600 mb-4 font-black">ACA Wholesale</p>
            ...
            <p className="text-gray-500 text-sm mb-8 max-w-xl mx-auto">Accédez à des lots de vêtements de marque sélectionnés avec soin, expédiés rapidement depuis la France.</p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link href="/produits" className="inline-block text-black px-8 md:px-10 py-4 font-black text-xs uppercase tracking-widest hover:opacity-90 transition-all" style={{ background: 'linear-gradient(135deg, #C4962A, #E8B84B)' }}>VOIR NOS LOTS →</Link>
              <Link href="/contact" className="inline-block border border-white/20 text-white px-8 md:px-10 py-4 font-black text-xs uppercase tracking-widest hover:border-white/50 transition-colors">NOUS CONTACTER</Link>
            </div>
          </div>
        </div>
      </section>
