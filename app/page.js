          ...
          <div className="grid grid-cols-3 lg:grid-cols-6 gap-2 md:gap-3">
            {categories.map((cat, i) => (
              <Link
                key={cat.name}
                href="/produits"
                className="group text-center p-4 md:p-6 bg-[#111] border border-white/07 hover:border-yellow-600/50 hover:-translate-y-1 transition-all duration-300 rounded"
        ...
        <div className="max-w-7xl mx-auto px-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
            {testimonials.map((t, i) => (
              <div key={i} className="bg-[#111] border border-white/07 p-6 md:p-8 rounded hover:border-yellow-600/30 hover:-translate-y-1 transition-all duration-300 fade-up" style={{ borderColor: 'rgba(255,255,255,0.07)', transitionDelay: `${i * 0.12}s` }}>
            ...
            {faqItems.map((item, i) => (
              <div key={i} className={`faq-item ${openFaq === i ? 'open-item' : ''}`}>
                ...
                >
                  ...
                  <span className="text-xl flex-shrink-0" style={{ color: '#C4962A', transition: 'transform 0.3s ease', display: 'inline-block', transform: openFaq === i ? 'rotate(45deg)' : 'rotate(0deg)' }}>+</span>
                </button>
