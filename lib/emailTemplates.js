  ...
  return `
    <tr>
      <td style="padding:12px 0; border-bottom:1px solid ${BORDER_COLOR}; font-size:14px;">
        <strong>${escapeHtml(p.name) || 'Lot'}</strong>
        ${p.description ? `<br><span style="color:${MUTED_COLOR};font-size:12px;">${escapeHtml(p.description)}</span>` : ''}
      </td>
...
function cartTotal(products) {
  return products.reduce((s, p) => s + (parseFloat(p.price) || 0) * (parseInt(p.qty || p.quantity) || 1), 0).toFixed(0)
}
