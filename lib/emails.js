...
function itemsRows(items) {
  return items.map(item => `
    <tr>
      <td style="padding:14px 16px;border-bottom:1px solid #1a1a1a;font-size:22px;width:44px;">${escapeHtml(item.emoji) || '📦'}</td>
      <td style="padding:14px 16px;border-bottom:1px solid #1a1a1a;color:#e5e7eb;font-weight:700;font-size:13px;">${escapeHtml(item.name)}</td>
      <td style="padding:14px 16px;border-bottom:1px solid #1a1a1a;color:#6b7280;font-size:12px;text-align:center;">×${parseInt(item.qty) || 0}</td>
      <td style="padding:14px 16px;border-bottom:1px solid #1a1a1a;color:#C4962A;font-weight:900;font-size:14px;text-align:right;white-space:nowrap;">${(parseFloat(item.price) || 0) * (parseInt(item.qty) || 0)}€</td>
    </tr>`).join('')
}
  ...
  const html = wrap(`
    <div style="background:#111;border:1px solid #1f1f1f;border-radius:12px;padding:36px;margin-bottom:20px;">
      ...
      <p style="color:#9ca3af;font-size:14px;line-height:1.6;margin:0 0 28px;">
        Bonjour <strong style="color:#fff;">${escapeHtml(prenom) || 'Revendeur'}</strong>, votre sélection de lots ACA Wholesale est toujours disponible. Il vous suffit d'un clic pour finaliser votre commande.
      </p>
...
export async function send8hReminder({ email, prenom, items, total }) {
  const html = wrap(`
    <div style="background:#111;border:1px solid #1f1f1f;border-radius:12px;padding:36px;margin-bottom:20px;">
      ...
      <p style="color:#9ca3af;font-size:14px;line-height:1.6;margin:0 0 28px;">
        Bonjour <strong style="color:#fff;">${escapeHtml(prenom) || 'Revendeur'}</strong>, vos lots sélectionnés vous attendent. Ne laissez pas passer cette opportunité de revente !
      </p>
...
export async function send16hReminder({ email, prenom, items, total }) {
  const html = wrap(`
    <div style="background:#111;border:1px solid rgba(239,68,68,0.25);border-radius:12px;padding:36px;margin-bottom:20px;">
      ...
      <p style="color:#9ca3af;font-size:14px;line-height:1.6;margin:0 0 28px;">
        Bonjour <strong style="color:#fff;">${escapeHtml(prenom) || 'Revendeur'}</strong>, votre panier va expirer très bientôt. Finalisez votre commande maintenant avant de perdre votre sélection.
      </p>
  ...
  const html = wrap(`
    <div style="background:#111;border:1px solid #1f1f1f;border-radius:12px;padding:36px;margin-bottom:20px;">
      ...
      <p style="color:#6b7280;font-size:14px;line-height:1.6;margin:0 0 24px;">
        Bonjour <strong style="color:#9ca3af;">${escapeHtml(prenom) || 'Revendeur'}</strong>, votre panier de <strong style="color:#9ca3af;">${total}€</strong> a été clôturé après 24h sans validation. Voici ce qu'il contenait :
      </p>
...
export async function sendOrderConfirmation({ email, prenom, orderId, items, total }) {
  const itemsBlock = Array.isArray(items) && items.length > 0
    ? `<table style="width:100%;border-collapse:collapse;background:#0d0d0d;border-radius:8px;overflow:hidden;margin-bottom:24px;">
        ${itemsRows(items)}
        <tr>
          <td colspan="3" style="padding:14px 16px;text-align:right;font-size:11px;color:#6b7280;text-transform:uppercase;font-weight:700;letter-spacing:0.06em;">Total</td>
          <td style="padding:14px 16px;text-align:right;font-weight:900;font-size:22px;color:#fff;white-space:nowrap;">${total}€</td>
        </tr>
      </table>`
    : `<div style="background:#0d0d0d;border-radius:8px;padding:16px;margin-bottom:24px;">
        <p style="color:#9ca3af;font-size:13px;line-height:1.6;margin:0 0 12px;">${items || '—'}</p>
        <p style="color:#6b7280;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;margin:0 0 4px;">Total</p>
        <p style="color:#fff;font-size:22px;font-weight:900;margin:0;">${total}€</p>
      </div>`

  const html = wrap(`
    <div style="background:#111;border:1px solid #1f1f1f;border-radius:12px;padding:36px;margin-bottom:20px;">
      <div style="background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.2);border-radius:6px;padding:11px 16px;margin-bottom:24px;">
        <p style="color:#22c55e;font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:0.08em;margin:0;">Commande confirmée</p>
      </div>
      ...
      <p style="color:#9ca3af;font-size:14px;line-height:1.6;margin:0 0 24px;">
        Bonjour <strong style="color:#fff;">${escapeHtml(prenom) || 'Revendeur'}</strong>, votre paiement a bien été reçu. Voici le récapitulatif de votre commande.
      </p>

      <div style="background:#0d0d0d;border-radius:8px;padding:14px 16px;margin-bottom:24px;display:inline-block;width:100%;box-sizing:border-box;">
        <p style="color:#6b7280;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;margin:0 0 6px;">Numéro de commande</p>
        <p style="color:#C4962A;font-size:20px;font-weight:900;letter-spacing:0.1em;margin:0;font-family:monospace;">${escapeHtml(orderId)}</p>
      </div>

      ${itemsBlock}

      <div style="background:rgba(196,150,42,0.07);border:1px solid rgba(196,150,42,0.18);border-radius:8px;padding:16px;margin-bottom:24px;">
        <p style="color:#C4962A;font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:0.08em;margin:0 0 6px;">Prochaines étapes</p>
        <p style="color:#9ca3af;font-size:13px;line-height:1.6;margin:0;">Votre commande sera traitée sous 24h. Vous recevrez un email avec votre numéro de suivi GLS dès l'expédition.</p>
      </div>

      <div style="text-align:center;">
        <a href="${SITE_URL}/compte" style="display:inline-block;background:linear-gradient(135deg,#C4962A,#E8B84B);color:#000;padding:16px 44px;font-weight:900;font-size:13px;text-transform:uppercase;letter-spacing:0.1em;border-radius:4px;text-decoration:none;">
          SUIVRE MA COMMANDE →
        </a>
      </div>
    </div>`)

  return sendEmail(email, `Confirmation de commande ${orderId} — ${total}€`, html)
}

// ─── Email admin : nouvelle commande ─────────────────────────────────────
export async function sendAdminOrderNotification({ orderId, clientName, email, total, items }) {
  const adminEmail = process.env.ADMIN_EMAIL || 'contact@aca-wholesale.com'
  const timestamp = new Date().toLocaleString('fr-FR', { timeZone: 'Europe/Paris', dateStyle: 'full', timeStyle: 'short' })

  const html = wrap(`
    <div style="background:#111;border:1px solid #1f1f1f;border-radius:12px;padding:36px;margin-bottom:20px;">
      <p style="color:#C4962A;font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:0.1em;margin:0 0 12px;">Nouvelle commande reçue</p>
      <h1 style="color:#C4962A;font-size:32px;font-weight:900;letter-spacing:0.08em;margin:0 0 24px;font-family:monospace;">${escapeHtml(orderId)}</h1>

      <table style="width:100%;border-collapse:collapse;margin-bottom:28px;">
        <tr>
          <td style="padding:10px 0;border-bottom:1px solid #1a1a1a;color:#6b7280;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;width:130px;">Client</td>
          <td style="padding:10px 0;border-bottom:1px solid #1a1a1a;color:#e5e7eb;font-size:13px;font-weight:700;">${escapeHtml(clientName) || '—'}</td>
        </tr>
        <tr>
          <td style="padding:10px 0;border-bottom:1px solid #1a1a1a;color:#6b7280;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;">Email</td>
          <td style="padding:10px 0;border-bottom:1px solid #1a1a1a;"><a href="mailto:${escapeHtml(email)}" style="color:#C4962A;text-decoration:none;font-size:13px;">${escapeHtml(email) || '—'}</a></td>
        </tr>
        <tr>
          <td style="padding:10px 0;border-bottom:1px solid #1a1a1a;color:#6b7280;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;">Total</td>
          <td style="padding:10px 0;border-bottom:1px solid #1a1a1a;color:#fff;font-size:22px;font-weight:900;white-space:nowrap;">${total}€</td>
        </tr>
        <tr>
          <td style="padding:10px 0;border-bottom:1px solid #1a1a1a;color:#6b7280;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;">Articles</td>
          <td style="padding:10px 0;border-bottom:1px solid #1a1a1a;color:#9ca3af;font-size:12px;line-height:1.6;">${items || '—'}</td>
        </tr>
        <tr>
          <td style="padding:10px 0;color:#6b7280;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;">Reçu le</td>
          <td style="padding:10px 0;color:#6b7280;font-size:12px;">${timestamp}</td>
        </tr>
      </table>

      <div style="text-align:center;">
        <a href="${SITE_URL}/admin" style="display:inline-block;background:linear-gradient(135deg,#C4962A,#E8B84B);color:#000;padding:16px 44px;font-weight:900;font-size:13px;text-transform:uppercase;letter-spacing:0.1em;border-radius:4px;text-decoration:none;">
          VOIR DANS L'ADMIN →
        </a>
      </div>
    </div>`)

  return sendEmail(adminEmail, `Nouvelle commande ${orderId} — ${total}€`, html)
}
...
export async function sendShippingNotification({ email, prenom, orderId, trackID, trackingUrl }) {
  const html = wrap(`
    <div style="background:#111;border:1px solid #1f1f1f;border-radius:12px;padding:36px;margin-bottom:20px;">
      ...
      <p style="color:#9ca3af;font-size:14px;line-height:1.6;margin:0 0 28px;">
        Bonjour <strong style="color:#fff;">${escapeHtml(prenom) || 'Revendeur'}</strong>, votre commande <strong style="color:#C4962A;">${escapeHtml(orderId)}</strong> a été expédiée via GLS. Vous pouvez suivre votre colis en temps réel.
      </p>
      ...
      <div style="background:#0d0d0d;border-radius:8px;padding:20px;margin-bottom:24px;text-align:center;">
        ...
        <p style="color:#fff;font-size:22px;font-weight:900;letter-spacing:0.15em;margin:0;">${escapeHtml(trackID)}</p>
      </div>
